...
export default function WorkspaceIndicator({ workspaceName = "Personal", onClick }: WorkspaceIndicatorProps) {
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:bg-zinc-900 dark:hover:bg-zinc-800 transition-colors group"
        >
            ...
            <ChevronRight className="w-3 h-3 text-zinc-400 group-hover:text-zinc-600 dark:text-zinc-400 dark:group-hover:text-zinc-300 transition-colors" />
        </motion.button>
    );
}
