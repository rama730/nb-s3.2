"use client";

import { Users, FolderKanban, FileText } from "lucide-react";
import { cn } from "@/lib/utils";
import { useReducedMotionPreference } from "@/components/providers/theme-provider";

interface ProfileStatsProps {
    stats: {
        connectionsCount: number;
        projectsCount: number;
        postsCount: number;
    };
}

export default function ProfileStats({ stats }: ProfileStatsProps) {
    const reduceMotion = useReducedMotionPreference();
    const statCards = [
        {
            label: "Connections",
            value: stats.connectionsCount || 0,
            icon: Users,
            color: "bg-[linear-gradient(90deg,var(--theme-gradient-start),var(--theme-gradient-end))]",
            bgColor: "bg-primary/10 dark:bg-primary/15",
            textColor: "text-primary",
        },
        {
            label: "Projects",
            value: stats.projectsCount || 0,
            icon: FolderKanban,
            color: "bg-[linear-gradient(90deg,var(--theme-gradient-start),var(--theme-gradient-end))]",
            bgColor: "bg-primary/10 dark:bg-primary/15",
            textColor: "text-primary",
        },
        {
            label: "Posts",
            value: stats.postsCount || 0,
            icon: FileText,
            color: "bg-[linear-gradient(90deg,var(--theme-gradient-start),var(--theme-gradient-end))]",
            bgColor: "bg-primary/10 dark:bg-primary/15",
            textColor: "text-primary",
        },
    ];

    return (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {statCards.map((stat, index) => {
                const Icon = stat.icon;
                return (
                    <div
                        key={stat.label}
                        className={cn(
                            "rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 hover:shadow-lg transition-all duration-300 cursor-pointer group",
                            !reduceMotion && "hover:scale-105 hover:-translate-y-1"
                        )}
                        style={{
                            animationDelay: `${index * 100}ms`,
                        }}
                    >
                        <div className="flex items-center justify-between mb-4">
                            <div className={cn("p-3 rounded-xl", stat.bgColor)}>
                                <Icon className={cn("w-6 h-6", stat.textColor)} />
                            </div>
                            <div className={cn("w-12 h-1 rounded-full opacity-20", stat.color)} />
                        </div>
                        <div className="space-y-1">
                            <p className={cn("text-3xl font-bold text-zinc-900 dark:text-white transition-transform duration-300", !reduceMotion && "group-hover:scale-110")}>
                                {stat.value.toLocaleString()}
                            </p>
                            <p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">{stat.label}</p>
                        </div>
                    </div>
                );
            })}
        </div>
    );
}
