...
export function ApplicationList() {
    const applications = useChatStore((state) => state.applications);
    const applicationsLoading = useChatStore((state) => state.applicationsLoading);
    const hasMoreApplications = useChatStore((state) => state.hasMoreApplications);
    const fetchApplications = useChatStore((state) => state.fetchApplications);
    const loadMoreApplications = useChatStore((state) => state.loadMoreApplications);
    ...
    const renderRow = (app: (typeof applications)[number]) => (
        <button
            key={app.id}
                ...
                <div className="flex flex-col gap-0.5">
                    ...
                    <p className="text-[10px] text-zinc-400 truncate flex items-center gap-1">
                        ...
                        {(app.lifecycleStatus || app.status) !== 'pending' && (
                            <span
                                className={`px-1 rounded-full text-[8px] font-bold uppercase ${
                                    (app.lifecycleStatus || app.status) === 'accepted'
                            ...
                            >
                                {(app.lifecycleStatus || app.status) === 'role_filled'
                                    ? 'filled'
                                    : (app.lifecycleStatus || app.status)}
                            </span>
                        )}
                    </p>
    ...
    return (
        <div className="h-full min-h-0 overflow-hidden">
            <Virtuoso
                style={{ height: '100%' }}
                data={applications}
                computeItemKey={(_, app) => app.id}
                increaseViewportBy={{ top: 220, bottom: 320 }}
                endReached={() => {
                    if (hasMoreApplications && !applicationsLoading) {
                    ...
                    Footer: () =>
                        hasMoreApplications ? (
                            <div className="py-3 text-center text-xs text-zinc-500">
                                {applicationsLoading ? 'Loading…' : 'Scroll for more applications'}
                            </div>
                        ) : null,
                }}
                itemContent={(_, app) => renderRow(app)}
            />
        </div>
    );
}
