...
// Ensure parseMessageSegments is imported
if (!fullCode.includes('parseMessageSegments')) {
    const importCodeSnippet = "import { parseMessageSegments } from '@/lib/messages/code-snippets';\n";
    const lastImportIndex = fullCode.lastIndexOf('import ');
    const endOfLastImport = fullCode.indexOf('\n', lastImportIndex);
    fullCode = fullCode.substring(0, endOfLastImport + 1) + importCodeSnippet + fullCode.substring(endOfLastImport + 1);
}

// Add code segment render import
if (!fullCode.includes('CodeSegmentV2')) {
    fullCode = fullCode.replace('    MessageTextContentV2,', '    MessageTextContentV2,\n    CodeSegmentV2,');
}

// Ensure useMemo is used for parsedSegments
const hookInjectStr = "    const [followUpDueAt, setFollowUpDueAt] = useState('');";
if (fullCode.includes(hookInjectStr) && !fullCode.includes('const parsedSegments = useMemo(')) {
    const parsedSegmentsHook = `\n    const parsedSegments = useMemo(() => parseMessageSegments(message.content || ''), [message.content]);`;
    fullCode = fullCode.replace(hookInjectStr, hookInjectStr + parsedSegmentsHook);
}
...
const replacement = `                        {(parsedSegments.length === 0 ? [{ type: 'empty', content: '' }] : parsedSegments).map((segment, index, arr) => {
                            ...
                            const renderPrefix = () => (
                                <>
                                    ...
                                    {(isPinned || isApplication || privateFollowUp) && isFirst ? (
                                        <div className="mb-1 flex items-center gap-2">
                                            ...
                                            {isApplication ? <span className={\`text-[10px] font-bold uppercase opacity-70 \${isOwn ? 'text-white' : 'text-zinc-500 dark:text-zinc-300'}\`}>Application status: {getApplicationStatusLabel(applicationStatus)}</span> : null}
                            ...
                            const renderSuffix = () => (
                                <>
                                    {structured ? <StructuredMessageCardV2 structured={structured} isOwn={isOwn} currentUserId={user?.id ?? null} creatorId={message.senderId ?? null} isActionLoading={workflowActionLoading} onResolveAction={structured.workflowItemId ? handleResolveWorkflow : undefined} /> : null}
                                    {!structured && contextChips.length > 0 ? <MessageContextChipRowV2 chips={contextChips} tone={isOwn ? 'inverted' : 'default'} compact /> : null}
                                    {linkedWork.length > 0 ? (
                                        <div className="mb-2 flex min-w-0 max-w-full flex-wrap items-center gap-1.5 overflow-hidden">
                                            ...
                                            {linkedWork.length > 2 ? <button type="button" onClick={() => setLinkedWorkExpanded((current) => !current)} className={cn('inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-semibold', isOwn ? 'bg-white/10 text-white/80 hover:bg-white/15' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700')}>{linkedWorkExpanded ? 'Show less' : \`\${linkedWork.length - 2} linked items\`} <ChevronDown className={cn('h-3 w-3 transition-transform', linkedWorkExpanded && 'rotate-180')} /></button> : null}
                                        </div>
                                    ) : null}
                                    {attachments.length > 0 ? <MessageAttachmentsV2 attachments={attachments} onContentLoad={onContentLoad} /> : null}
                                    {renderedLinkPreview ? <LinkPreviewCard preview={renderedLinkPreview} isOwn={isOwn} loading={!linkPreview} onContentLoad={onContentLoad} /> : null}
                                </>
                            );
                            ...
                            if (segment.type === 'code') {
                                return (
                                    <React.Fragment key={index}>
                                        {isFirst && (message.replyTo || isPinned || isApplication || privateFollowUp || isFocusedReplyTarget) && (
                                            <div className="msg-bubble-shell !bg-transparent !p-0 !border-0 !shadow-none">
                                                {renderPrefix()}
                                            </div>
                                        )}
                                        <CodeSegmentV2
                                            ...
                                            isOwn={isOwn}
                                        />
                                        {isLast && (structured || contextChips.length > 0 || linkedWork.length > 0 || attachments.length > 0 || renderedLinkPreview) && (
                                            <div className="msg-bubble-shell !bg-transparent !p-0 !border-0 !shadow-none">
                                                {renderSuffix()}
                                            </div>
                                        )}
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        isApplication || (segment.type === 'empty' && parsedSegments.length === 1 && segment.content === '') ? '!bg-transparent !bg-none !border-0 !shadow-none !p-0' : cn(
                                            isOwn
                                                ? cn('rounded-2xl', (showAvatar || (index > 0)) && 'rounded-br-sm', index < arr.length - 1 && 'rounded-tr-sm')
                                                : cn('rounded-2xl', (showAvatar || (index > 0)) && 'rounded-bl-sm', index < arr.length - 1 && 'rounded-tl-sm'),
                                    ...
                                    style={{
                                        boxShadow: (isFocusedReplyTarget && isFirst) ? undefined : 'var(--msg-shadow)',
                                ...
                                >
                                    ...
                                    {isEditing ? (
                                        <div className="rounded-xl border border-zinc-200 bg-white p-2 dark:border-zinc-700 dark:bg-zinc-900">
                                            <textarea value={draftContent} onChange={(event) => setDraftContent(event.target.value)} rows={3} maxLength={4000} className="w-full rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-ring dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
...
fullCode = fullCode.substring(0, startIndex) + replacement + fullCode.substring(endIndex);

fs.writeFileSync('/Users/chrama/Downloads/nb-s3/src/components/chat/v2/MessageBubbleV2.tsx', fullCode);
console.log('Successfully replaced MessageBubbleV2.tsx content with precise mapping.');
