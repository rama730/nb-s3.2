...
test.describe("Messaging unread rebound", () => {
  ...
  test("cleared unread badge does not reappear without a new message", async ({ browser }) => {
    ...
    const unreadRow = page
      .locator("[data-testid^='conversation-row-'][data-message-attention='new']")
      .first();
    ...
    await expect(selectedRow).toHaveAttribute("data-message-attention", "none", { timeout: 10000 });

    // Hold briefly to catch delayed stale refresh rebounds.
    await page.waitForTimeout(3000);
    await expect(selectedRow).toHaveAttribute("data-message-attention", "none");
