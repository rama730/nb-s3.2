...
-- This file is retained as a reviewable SQL snapshot and is not the mutating
-- source of truth for database security changes. Apply Drizzle migrations
...
-- Canonicalize profile privacy, isolate recovery-code state, and tighten
-- message insert membership checks.
-- ============================================================================

CREATE TABLE IF NOT EXISTS public.profile_security_states (
...
CREATE INDEX IF NOT EXISTS profile_security_states_generated_at_idx
    ON public.profile_security_states(recovery_codes_generated_at);
...
BEGIN
  IF EXISTS (
    ...
    INSERT INTO public.profile_security_states (user_id, security_recovery_codes, recovery_codes_generated_at, updated_at)
    SELECT
      id,
    ...
    ALTER TABLE public.profiles DROP COLUMN IF EXISTS security_recovery_codes;
    ALTER TABLE public.profiles DROP COLUMN IF EXISTS recovery_codes_generated_at;
  END IF;
END $$;

ALTER TABLE public.profile_security_states ENABLE ROW LEVEL SECURITY;
