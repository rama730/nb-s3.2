...
5. WHEN the user deletes a node, THE Files_Tab SHALL move the node to trash (soft delete), confirm the action with a single confirmation step, and SHALL NOT expose a "Delete permanently" affordance inside the Files_Tab. Permanent deletion is reached from a dedicated trash UI that is out of scope for the Files_Tab (see design.md § Open Question 3 — Trash surface removed from the Files tab).
